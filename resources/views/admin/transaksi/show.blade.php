...
<div class="container mx-auto px-6 py-8">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('admin.transaksi.index') }}" class="text-orange-500 hover:text-orange-600 font-semibold mb-4 inline-block">
            ← Kembali ke Daftar Transaksi
        </a>
        <h2 class="text-3xl font-bold text-gray-800">Detail Transaksi #{{ $transaksi->transaksi_id }}</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between pb-4 border-b">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ optional($transaksi->user)->name ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ optional($transaksi->menu)->nama_menu ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ \Carbon\Carbon::parse($transaksi->waktu_transaksi)->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            <p class="text-2xl font-bold text-gray-800">{{ $transaksi->jumlah }}x</p>
                        </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-md p-6">
                ...
                <div class="mb-6">
                    @php
                        ...
                        $currentStatus = $statusConfig[$transaksi->status_transaksi] ?? $statusConfig['menunggu_konfirmasi'];
                    @endphp
                ...
                <!-- Update Status Form -->
                <form method="POST" action="{{ route('admin.transaksi.updateStatus', $transaksi->transaksi_id) }}">
                    @csrf
                    <div class="mb-4">
                        ...
                        <select name="status_transaksi" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-orange-500 focus:outline-none">
                            <option value="menunggu_konfirmasi" {{ $transaksi->status_transaksi=='menunggu_konfirmasi' ? 'selected' : '' }}>⏳ Menunggu Konfirmasi</option>
                            <option value="sedang_dibuat" {{ $transaksi->status_transaksi=='sedang_dibuat' ? 'selected' : '' }}>🔄 Sedang Dibuat</option>
                            <option value="siap" {{ $transaksi->status_transaksi=='siap' ? 'selected' : '' }}>✅ Siap Diambil</option>
                            <option value="selesai" {{ $transaksi->status_transaksi=='selesai' ? 'selected' : '' }}>✔️ Selesai</option>
            ...
            @if($transaksi->status_transaksi == 'sedang_dibuat')
            <div class="bg-white rounded-2xl shadow-md p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4">Aksi Cepat</h3>
